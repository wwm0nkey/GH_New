...
public class GrapplingHook : MonoBehaviour
{
    ...
    public GameObject grappling_line;
    ...
    void Update()
    {
        ...
        {
            this.GetComponent<PlayerController>().gravity = 12.81f;
            destorysphere = false;
            ...
            if (Physics.Raycast(rayOrigin, Camera.main.transform.forward, out raycast_hit, rayDistance) && raycast_hit.collider.tag != "NoGrapple" || raycast_hit.collider.tag != "Player")
            {
                ...
                hook.transform.parent = raycast_hit.collider.transform;
                hook.name = transform.name + " Hook";
                this.GetComponent<GravityBody>().planet = hook.GetComponent<GravityAttractor>();
                hooked_node = hook;
        ...
        {
            this.GetComponent<PlayerController>().gravity = 100;
            Destroy(hook);
            BreakTether();
            destorysphere = true;
